...
from project.lit_qa import QAAdapterModel
...
# f"model/xlm/drcd/squad_adapter/drcd-squad-adapter-epoch={i:02d}.ckpt"
# f"model/mbert/drcd/squad_adapter/drcd-squad-adapter-epoch={i:02d}.ckpt"
# f"model/xlm/madx/madx-epoch={i:02d}.ckpt"
# f"model/mbert/madx/madx-epoch={i:02d}.ckpt"
...
def evaluate():
    ...
    for i in range(30):
        ckpt_path = f"model/mbert/drcd/squad_adapter/drcd-squad-adapter-epoch={i:02d}.ckpt"
        model = QAAdapterModel.load_from_checkpoint(
            ckpt_path,
            hf_path=MODEL_NAME, 
...
def main():
    ...
    callbacks = [
        ModelCheckpoint(
            dirpath="model/xlm/drcd/squad_adapter",
            filename="drcd-squad-adapter-{epoch:02d}",
            save_top_k=-1,
        )
    ]
    trainer = Trainer(
    ...
    model = QAAdapterModel(hf_path=MODEL_NAME, eval_examples=dm.val_examples(), eval_dataset=dm.val_dataset())
    model.train_adapter()
    model.activate("en")
    from torchinfo import summary
    summary(model)
    trainer.fit(model, datamodule=dm, ckpt_path="/user_data/unans_qa/model/xlm/drcd/squad_adapter/drcd-squad-adapter-epoch=14.ckpt")
